...
import { isRetryableAttemptStatus } from "./routing/index.ts";
import {
...
import { resolveResetWindowConfig } from "./combo/quotaScoring.ts";
import { preScreenTargets } from "./combo/quotaStrategies.ts";
import {
  resolveQuotaExhaustionCutoffForTarget,
} from "./combo/quotaExhaustionCutoff.ts";
import { buildAutoCandidates } from "./combo/autoCandidates.ts";
...
// The auto-strategy candidate builder lives in combo/autoCandidates.ts. Re-exported so existing
// importers of combo.ts (chat handlers, tests) keep resolving it from here.
export { buildAutoCandidates };
      ...
      const executeTarget = async (
        ...
        for (let retry = 0; retry <= maxRetries; retry++) {
          ...
          const isTransient =
            ...
            !isTokenLimitBreach &&
            !scopedFailure &&
            isRetryableAttemptStatus(result.status);
