...
| `Unit Tests fast-path`                           | Code PRs; 5-shard unit suite                                                                                                                                                                                     | Yes                                                                                       |
...
| `check:api-governance`            | Every `src/app/api` route and exported handler verb is documented in `docs/openapi.yaml`, every operation carries `x-stability` / `x-owner` / `x-since` / `x-rate-limit`, stable operations have a contract test and an example, deprecated ones have `x-sunset`, and `docs/reference/API_USE_CASES.md` names real operations (policy: `docs/architecture/API_GOVERNANCE.md`). Ratchet: routes with no test reference and phantom operations frozen in `config/quality/api-governance-baseline.json` can only shrink. Also in the `quality.yml` Fast Quality Gates loop.                                                | Yes                                      |
...
| `check:openapi-coverage`       | OpenAPI spec documents 100% of real routes (no new route without docs)                                                                            | Yes                        |
